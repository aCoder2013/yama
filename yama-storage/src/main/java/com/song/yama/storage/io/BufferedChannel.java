...
public class BufferedChannel extends BufferedReadChannel implements Closeable {
    ...
    /**
     * Write all the data in src to the {@link FileChannel}. Note that this function can buffer or re-order writes based
     * on the implementation. These writes will be flushed to the disk only when flush() is invoked.
     *
     ...
     *
     * @param forceMetadata - If true then this method is required to force changes to both the file's content and
    ...
    /**
     * Write any data in the buffer to the file and advance the writeBufferPosition. Callers are expected to synchronize
     * appropriately
     *
